...
class TransactionAdapter(
    ...
    inner class TransactionViewHolder(private val binding: ItemTransactionBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(transaction: Transaction) {
            // ERROR FIX: Ab binding ka istemal kiya ja raha hai
            ...
            binding.transactionAmount.text = "₹${transaction.amount.toInt()}"

            val context = itemView.context
            // ERROR FIX: Rangon ke naye naam istemal kiye ja rahe hain
            if (type == "expense") {
                binding.transactionAmount.setTextColor(ContextCompat.getColor(context, R.color.red_pending))
            } else {
                binding.transactionAmount.setTextColor(ContextCompat.getColor(context, R.color.green_paid))
            }
        }
    }
}
