...
class YearlyGridActivity : AppCompatActivity(), YearlyGridAdapter.OnCellClickListener {
    ...
    private fun setupRecyclerView() {
        ...
        binding.recyclerViewGrid.adapter = YearlyGridAdapter(memberList, selectedYear, this)
    }

    private fun fetchMembers() {
        db.collection("chanda_data/shared/members").orderBy("name")
            .addSnapshotListener { snapshots, e ->
                // ERROR FIX: 'nil' ko 'null' se badal diya gaya hai
                if (e != null) { return@addSnapshotListener }
                memberList.clear()
                snapshots?.forEach { doc -> memberList.add(doc.toObject(Member::class.java).copy(id = doc.id)) }
                binding.recyclerViewGrid.adapter?.notifyDataSetChanged()
            }
    }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        return when (item.itemId) {
            ...
            // PDF export ka logic yahan add kiya ja sakta hai
