package com.masjid.app.services
...
class AzanPlaybackService : Service() {
    ...
    private fun createNotification(): Notification {
        ...
        val pauseResumeIntent = PendingIntent.getService(this, 0, Intent(
            this,
            AzanPlaybackService::class.java
        ).apply { Intent.setAction = ACTION_PAUSE_RESUME }, PendingIntent.FLAG_IMMUTABLE)
        val stopIntent = PendingIntent.getService(this, 1, Intent(
            this,
            AzanPlaybackService::class.java
        ).apply { Intent.setAction = ACTION_STOP }, PendingIntent.FLAG_IMMUTABLE)

        val pauseActionText = if (isPlaying) "Pause" else "Resume"
        val pauseActionIcon = if (isPlaying) R.drawable.ic_media_pause else R.drawable.ic_media_play

        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setContentTitle("Azan: $prayerName")
            .setContentText(if (isPlaying) "Azan ho rahi hai..." else "Azan ruki hui hai.")
            .setSmallIcon(com.masjid.app.R.drawable.ic_alert_bell)
