        ...
        fun bind(member: Member) {
            ...
            when {
                paidAmount >= monthlyAmount && monthlyAmount > 0 -> {
                    binding.textViewStatus.text = "✔ जमा"
                    binding.textViewStatus.setTextColor(ContextCompat.getColor(context, R.color.green_paid))
                }
                paidAmount > 0 -> {
                    binding.textViewStatus.text = "₹${pendingAmount.toInt()} बकाया"
                    binding.textViewStatus.setTextColor(ContextCompat.getColor(context, R.color.orange_partial))
                }
                else -> {
                    binding.textViewStatus.text = "₹${monthlyAmount.toInt()} बकाया"
                    binding.textViewStatus.setTextColor(ContextCompat.getColor(context, R.color.red_pending))
                }
            }
